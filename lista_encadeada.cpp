...
void InserirInicio(int num)
{
	ElementoDaLista_Simples *NovoElemento; // alocando em um novo espaço da memoria antes de inserir na lista
	NovoElemento = (struct ElementoDaLista_Simples *)malloc(sizeof(struct ElementoDaLista_Simples));
	NovoElemento->dado = num; // o campo dado ->(significa campo) ira receber num
